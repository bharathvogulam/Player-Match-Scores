...
const convertMatchscoreToResponseObject = (dbObject) => {
  return {
...
app.get("/players/", async (request, response) => {
  ...
  response.send(
    stateArray.map((eachstate) => convertPlayerDbToResponseObject(eachstate))
  );
});
...
app.get("/players/:playerId/", async (request, response) => {
  ...
  response.send(convertPlayerDbToResponseObject(stateArray));
});
...
app.get("/matches/:matchId/", async (request, response) => {
  ...
  const getMovieQuery = `
        ...
        WHERE 
            match_id=${matchId};`;
  const stateArray = await database.get(getMovieQuery);
  response.send(convertMatchDbToResponseObject(stateArray));
});
